public class CoinChange {
	...
	public int numberOfWaysToMakeChange(int[] coins, int amount) {
		...
		int[][] dp = new int[coins.length + 1][amount + 1]; // add 1 for zero amount as base case
		...
		for (int i = 1; i < dp.length; i++) {
			...
			for (int j = 1; j < dp[0].length; j++) {
				if (j >= coins[i - 1]) { // when amount is greater than coin
					dp[i][j] = dp[i - 1][j] + dp[i][j - coins[i - 1]]; // when coin not choosed + (coin choosed so add)
																		// optimized value for the balance amount
				} else {
